...
RSpec.describe "Admin::V1::Categories", type: :request do
  ...
  context "GET /categories" do
    ...
    it "return all categories" do
      ...
      expect(body_json['categories']).to contain_exactly *categories.as_json(
        only: [:id, :name]
      )
    end
  ...
  context "POST /categories" do
    ...
    context "with valid params" do
      ...
      it "returns last added Category" do
        post url, headers: auth_header(user), params: category_params
        expected_category = Category.last.as_json(only: [:id, :name])
    ...
    context "with invalid params" do
      ...
      it "doesn't add a new Category" do
        expect do
  ...
  context "PATCH /categories/:id" do
    ...
    context "with valid params" do
      ...
      it "returns updated category" do
        patch url, headers: auth_header(user), params: category_params
        category.reload
        expected_category = category.as_json(only: [:id, :name])
        expect(body_json['category']).to eq expected_category
      end
